...
function buildExecSummary(ranking, gdpRank, infRank, totalGDP, avgGrowth, avgInflation, growingCount, kpiYear) {
  if (!ranking.length) return '';

  const topGrower    = ranking[0];
  const bottomGrower = ranking[ranking.length - 1];
  const highInflation = infRank.filter(c => c.value > 8);
  const lowInflation  = infRank.filter(c => c.value < 2);

  // Para 1 — big picture
  let p1 = `The G20 collectively produced <strong>${A.fmtGDP(totalGDP)}</strong> in <strong>${kpiYear}</strong>, with <strong>${growingCount}</strong> of ${ranking.length} member economies recording positive growth.`;
  if (growingCount === ranking.length) {
    p1 += ' All G20 members are currently in expansion.';
  } else if (growingCount < ranking.length / 2) {
    p1 += ' Growth is fragile across the bloc, with contraction or stagnation in the majority of members.';
  }
  if (avgGrowth != null) {
    p1 += ` Average GDP growth across the group stood at <strong>${A.fmtPct(avgGrowth)}</strong>.`;
  }

  // Para 2 — top / bottom performers
  let p2 = '';
  if (topGrower && bottomGrower && topGrower.iso3 !== bottomGrower.iso3) {
    p2 = `<strong>${topGrower.flag} ${A.escapeText(topGrower.name)}</strong> led G20 growth at <strong>${A.fmtPct(topGrower.value)}</strong>, `;
    if (bottomGrower.value < 0) {
      p2 += `while <strong>${bottomGrower.flag} ${A.escapeText(bottomGrower.name)}</strong> posted a contraction of <strong>${A.fmtPct(Math.abs(bottomGrower.value))}</strong>.`;
    } else {
      p2 += `while <strong>${bottomGrower.flag} ${A.escapeText(bottomGrower.name)}</strong> posted the weakest reading at <strong>${A.fmtPct(bottomGrower.value)}</strong>.`;
    }
  }

  // Para 3 — inflation
  let p3 = '';
  if (avgInflation != null) {
    p3 = `Inflation averaged <strong>${A.fmtPct(avgInflation)}</strong> across the G20.`;
    if (highInflation.length > 0) {
      const worst = highInflation[0];
      p3 += ` Elevated price pressures persist in <strong>${highInflation.length}</strong> member${highInflation.length > 1 ? 's' : ''}, notably <strong>${worst.flag} ${A.escapeText(worst.name)}</strong> at <strong>${A.fmtPct(worst.value)}</strong>.`;
    } else if (lowInflation.length > 0) {
      p3 += ` Deflationary conditions are a concern in <strong>${lowInflation.length}</strong> econom${lowInflation.length > 1 ? 'ies' : 'y'}.`;
    } else {
      p3 += ' Inflation is broadly contained within target ranges across the group.';
    }
  }

  const paras = [p1, p2, p3].filter(Boolean).map(p =>
    `<p class="exec-summary__para">${p}</p>`
  ).join('');

  return `
    <div class="exec-summary chart-card hc-enter">
      <div class="exec-summary__label">G20 Economic Snapshot — ${A.escapeText(String(kpiYear))}</div>
      <div class="exec-summary__body">${paras}</div>
    </div>
  `;
}
...
function renderOverview() {
  ...
  return `
    ...
    ${buildExecSummary(ranking, gdpRank, infRank, totalGDP, avgGrowth, avgInflation, growingCount, kpiYear)}

    <div class="chart-pair--overview hc-enter">
