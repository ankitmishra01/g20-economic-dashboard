// World Bank data endpoint — now reads from Supabase (seeded by sync/seed.js).
// Kept for debugging: GET /api/wb?indicator=GDP_GROWTH
// Falls back to live World Bank API if Supabase has no data for this indicator.
...
module.exports = async function handler(req, res) {
  ...
  try {
    const r = await fetch(
      `${SUPABASE_URL}/rest/v1/g20_economic_data?indicator_key=eq.${encodeURIComponent(indicator)}&select=country_iso3,year,value&order=year.asc`,
      {
        headers: {
          'apikey': SUPABASE_KEY,
